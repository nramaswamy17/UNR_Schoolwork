# humanoid_n1_task/humanoid_n1_env_cfg.py
...
def spawn_random_action_joints(env, env_ids):
    """Randomize ONLY 13 controlled joints around default, but safe."""
    ...
    # randomize action joints safely inside their limit range
    for i, jname in enumerate(all_joint_names):
        if jname in N1_ACTION_JOINTS_13:
            s = float(ACTION_SCALE_BY_JOINT[jname])
            #delta = (0.4 * s) * (2.0 * torch.rand((num_envs,), device=env.device) - 1.0)  # [-0.4s, +0.4s]
            delta = (0.05 * s) * (2.0 * torch.rand((num_envs,), device=env.device) - 1.0)
            qpos[:, i] = qpos[:, i] + delta
...
def randomize_root_pose_small(env, env_ids, roll_pitch_deg: float = 1.5, yaw_deg: float = 5.0):
...
def obs_left_foot_speed_x(env) -> torch.Tensor:
    robot = env.scene["robot"]
    ...
    if li < 0:
        return torch.zeros((env.num_envs, 1), device=env.device, dtype=torch.float32)
    return vel[:, li, 0:1].to(torch.float32)


def obs_right_foot_speed_x(env) -> torch.Tensor:
    robot = env.scene["robot"]
    if not hasattr(robot.data, "body_lin_vel_w"):
        return torch.zeros((env.num_envs, 1), device=env.device, dtype=torch.float32)
    vel = robot.data.body_lin_vel_w
    ...
    if ri < 0:
        return torch.zeros((env.num_envs, 1), device=env.device, dtype=torch.float32)
    return vel[:, ri, 0:1].to(torch.float32)


def obs_left_foot_speed_y(env) -> torch.Tensor:
    robot = env.scene["robot"]
    if not hasattr(robot.data, "body_lin_vel_w"):
        return torch.zeros((env.num_envs, 1), device=env.device, dtype=torch.float32)
    vel = robot.data.body_lin_vel_w
    ...
    if li < 0:
        return torch.zeros((env.num_envs, 1), device=env.device, dtype=torch.float32)
    return vel[:, li, 1:2].to(torch.float32)


def obs_right_foot_speed_y(env) -> torch.Tensor:
    robot = env.scene["robot"]
    if not hasattr(robot.data, "body_lin_vel_w"):
        return torch.zeros((env.num_envs, 1), device=env.device, dtype=torch.float32)
    vel = robot.data.body_lin_vel_w
    ...
    if ri < 0:
        return torch.zeros((env.num_envs, 1), device=env.device, dtype=torch.float32)
    return vel[:, ri, 1:2].to(torch.float32)
...
class ObservationsCfg:
    ...
    class CriticCfg(ObsGroup):
        ...
        lf_speed_x = ObsTerm(func=obs_left_foot_speed_x)
        rf_speed_x = ObsTerm(func=obs_right_foot_speed_x)
        lf_speed_y = ObsTerm(func=obs_left_foot_speed_y)
        rf_speed_y = ObsTerm(func=obs_right_foot_speed_y)
...
class RewardsCfg:
    ...
    upright = RewTerm(func=rew_upright_from_gravity, weight=10.0, params={"asset_cfg": SceneEntityCfg("robot")})
    height = RewTerm(func=rew_base_height_bonus, weight=6.0, params={"asset_cfg": SceneEntityCfg("robot"), "target_height": 0.70})
    pose = RewTerm(func=rew_pose_similarity_13, weight=10.0, params={"asset_cfg": SceneEntityCfg("robot", joint_names=N1_ACTION_JOINTS_13)})

    lin_vel = RewTerm(func=rew_lin_vel_penalty, weight=-0.3, params={"asset_cfg": SceneEntityCfg("robot")})
    ang_vel = RewTerm(func=rew_ang_vel_penalty, weight=-0.8, params={"asset_cfg": SceneEntityCfg("robot")})
    dof_vel = RewTerm(func=rew_dof_vel_penalty, weight=-0.02, params={"asset_cfg": SceneEntityCfg("robot", joint_names=N1_ACTION_JOINTS_13)})
    action_rate = RewTerm(func=rew_action_rate_penalty, weight=-0.02)
...
# =============================================================================
@configclass
class HumanoidN1EnvCfg(ManagerBasedRLEnvCfg):
    scene: HumanoidN1SceneCfg = HumanoidN1SceneCfg(num_envs=1024, env_spacing=2.0)
